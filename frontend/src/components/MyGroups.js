...
const MyGroups = () => {
  ...
  const acceptHandler = async (e) => {
    ...
    // await  axios.post("http://54.153.78.74:3010/accInvStatus", { acceptedGroup }).then((response) => {
  ...
  const rejectHandler = async (e) => {
    ...
    redux_inviteList.splice(e.target.dataset.id, 1);
    setDone(!done);
    // await  axios.post("http://54.153.78.74:3010/rejInvStatus", { rejectedGroup }).then((response) => {
    //   if (response.status === 269) {
    //     let newInviteList = [...inviteList];
    //     newInviteList.splice(e.target.dataset.id, 1);
    //     setInviteList(newInviteList);
